...
def get_transformed_data():
	...
	df_train = pd.read_csv('train54.csv', header = None)
	df_test = pd.read_csv('test54.csv', header = None)
...
#	np.random.shuffle(data_test)
	...
	X_train = np.reshape(X_train, [ N_train, 54, 54, 1], order = "F")
	N_test = X_test.shape[0];
	X_test = np.reshape(X_test, [ N_test, 54, 54, 1], order = "F")
